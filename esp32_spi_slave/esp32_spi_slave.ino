...
// ============================================================================
// BMS (charger_bq25792 module)
// ============================================================================
static const int I2C_SDA = 8;
static const int I2C_SCL = 9;
static const uint32_t BMS_SAMPLE_MS = 30000;  // Send battery every 30 seconds
static uint32_t lastBmsSampleMs = 0;
static uint32_t lastChargingCheckMs = 0;
static bool bmsInitialized = false;
static bool lastIsCharging = false;  // For Teensy BATTERY_CHARGING/NOT_CHARGING on change
...
static bool sendBatteryPacket() {
  ...
  float v = Charger_GetVBATDisplay_V();
  ...
  if (bmsInitialized && (now - lastChargingCheckMs >= 100)) {
    ...
    if (chg != lastIsCharging) {
      ...
      if (chg) {
        Serial1.println("BATTERY_CHARGING");
        Serial1.flush();
        Serial.println("[BMS] CHARGING → Remote Teensy");
      } else {
        Serial1.println("BATTERY_NOT_CHARGING");
        Serial1.flush();
        Serial.println("[BMS] NOT CHARGING → Remote Teensy");
      }
    }
  }
